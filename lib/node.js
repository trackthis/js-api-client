    ...
    login: function (data) {
      ...
      return checkTransport()
        ...
        .then(function(response) {
          ...
          // Calculate the signature if not given & possible
          if ( !signature && password ) {

            // Calculate the amount of iterations for key generation for this username
            var iterations = 0,
                hash       = crypto.createHash(signatureConfig.iterations.hash).update(data.username).digest('hex');
            while(hash.length) {
              iterations = ((iterations * 16) + parseInt(hash.substr(0,1), 16)) % signatureConfig.iterations.modulo;
              hash       = hash.substr(1);
            }
            iterations += signatureConfig.iterations.base;
